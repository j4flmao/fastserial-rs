...
**Field matching** — length-bucketed dispatch:

Fields are grouped by their byte-length, then dispatched:

```rust
// Generated by proc-macro
match key_bytes.len() {
    3 => match key_bytes {
        b"bar" => { f_bar = Some(Decode::decode(r)?); }
        b"foo" => { f_foo = Some(Decode::decode(r)?); }
        _ => { skip_value(r)?; }
    },
    5 => match key_bytes {
        b"hello" => { f_hello = Some(Decode::decode(r)?); }
        _ => { skip_value(r)?; }
    },
    _ => { skip_value(r)?; },
}
...
The outer `match key_bytes.len()` is a single integer compare. Inner byte-literal
patterns (`b"foo"`) are lowered by LLVM to integer loads + compares — **no memcmp
call**. For buckets with more than 3 fields, the inner match is replaced with
`binary_search_key()` for O(log n) average-case performance.

**Binary format**: O(1) — fields encoded in declaration order, no names stored.
